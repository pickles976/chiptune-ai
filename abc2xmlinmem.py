...
def mergePartMeasure (part, msre, ovrlaynum, rOpt): # merge msre into last measure of part, only for overlays
    ...
    mergeMeasure (last_msre, msre, slur_offset, ovrlaynum, rOpt) # voice offset = s.overlayVNum
...
def translateABC(abctext,filename):
    import time
    t_start = time.time()
    pad = "."
    xml_docs = getXmlDocs (abctext, 0, 1, False, False, False)
    for itune, xmldoc in enumerate (xml_docs):
        fnmNum = '%02d' % (itune + 1) if len (xml_docs) > 1 else ''
        writefile (pad, filename, "", xmldoc, False, False)
    print ('done in %.2f secs' % (time.time () - t_start))
